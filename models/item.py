...
class ItemModel(db.Model):  # tell SQLAlchemy entity that this is a model
    __tablename__ = 'items'

    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(80))
    price = db.Column(db.Float(precision=2))  # number of digits after decimal
    # after adding the StoreModel
    store_id = db.Column(db.Integer, db.ForeignKey('stores.id'))
    store = db.relationship('StoreModel')  # connect to StoreModel
